	...
	public function manual_verify_user() {
		if ( isset( $_GET['user_id'] ) && isset( $_GET['wp_nonce'] ) && wp_verify_nonce( $_GET['wp_nonce'], 'wc_email' ) ) {
			if ( isset( $_GET['wc_confirm'] ) && 'true' == $_GET['wc_confirm'] ) {
				...
				delete_user_meta( $_GET['user_id'], 'wcemailverified' );
...
//				'role'       => $user_role,
	...
	public function insert_plugin_email_services_notice() {
		if ( isset( $_GET['xl-email-services'] ) && 'yes' == $_GET['xl-email-services'] ) {
	...
	function filter_users_by_email_verification_section( $query ) {
		...
		if ( is_admin() &&
		     'users.php' == $pagenow &&
		     isset( $_GET['verification_status'] ) &&
		     is_array( $_GET['verification_status'] )
		) {
			$status = $_GET['verification_status'];
			$status = $status[0];
			if ( '' != $status ) {
