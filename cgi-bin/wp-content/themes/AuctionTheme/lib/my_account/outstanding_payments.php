...
function AuctionTheme_my_account_outstanding_payments_area_function()
{
...
<div id="content" class="account-content-area col-xs-12 col-sm-8 col-lg-9">
         ...
         <div class="my_box3">
            	...
            	<div class="box_title"><?php _e("Outstanding Payments","AuctionTheme"); ?></div>
				...
				$querystr = "
				 ...
				 ?>
                     ...
                     <?php
							...
							$asd_paid_items = 1;
                      			auctionTheme_get_post_outstanding_payment($post->bid_id);
		...
		if($my_page > 1)
		{
			echo '<a href="'.AuctionTheme_get_browse__special_pg_link('AuctionTheme_my_account_otstanding_pay_id', $previous_pg).'"><< '.__('Previous','AuctionTheme').'</a>';
			echo '<a href="'.AuctionTheme_get_browse__special_pg_link('AuctionTheme_my_account_otstanding_pay_id', $start_me).'"><<</a>';
		}
		//------------------------
		//echo $start." ".$end;
		for($i = $start; $i <= $end; $i ++) {
			if ($i == $pages_curent) {
				...
				echo '<a href="'.AuctionTheme_get_browse__special_pg_link('AuctionTheme_my_account_otstanding_pay_id', $i).'">'.$i.'</a>';
		...
		echo '<a href="'.AuctionTheme_get_browse__special_pg_link('AuctionTheme_my_account_otstanding_pay_id', $end_me).'">>></a>';

		if($page < $totalPages)
		echo '<a href="'.AuctionTheme_get_browse__special_pg_link('AuctionTheme_my_account_otstanding_pay_id', $next_pg).'">'.__('Next','AuctionTheme').' >></a>';
